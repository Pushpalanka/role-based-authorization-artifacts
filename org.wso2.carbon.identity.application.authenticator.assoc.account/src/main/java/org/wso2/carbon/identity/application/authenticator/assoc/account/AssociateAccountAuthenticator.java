...
public class AssociateAccountAuthenticator extends AbstractApplicationAuthenticator
    ...
    private static final String CURRENT_AUTHENTICATED_IDP_QUERY_PARAM = "idp";
    ...
    public AuthenticatorFlowStatus process(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            localUsername =
                    userProfileAdmin.getNameAssociatedWith(getAuthenticatedIDP(context), getAuthenticatedUser(context));
        } catch (UserProfileException e) {
            throw new AuthenticationFailedException("Error occurred while retrieving associated accounts", e);
        }
        if(localUsername!=null){
            context.setSubject(localUsername);
            return  AuthenticatorFlowStatus.SUCCESS_COMPLETED;
        }
    ...
    protected void processAuthenticationResponse(HttpServletRequest request, HttpServletResponse response,
        ...
        if (username != null && password != null) {
            ...
            String authenticatedUsername = getAuthenticatedUser(context);
            ...
            try {
                ...
                userProfileAdmin.associateID(getAuthenticatedIDP(context), getAuthenticatedUser(context));
    ...
    private String getAuthenticatedUser(AuthenticationContext context)
            throws AuthenticationFailedException {
        ...
        String authenticatedUsername = firstIDP.getValue().getUsername();
        if (log.isDebugEnabled()) {
    ...
    private String getAssociatedAccounts(String localUsername) throws UserProfileException {
        ...
        for (int i = 0; i < associatedAccounts.length; i++) {
            //associatedAccounts[i] format <IDP name>:username change to <IDP name>/username format with
            // String replace and building user list query params which needs to display on select page
            userIDs.append("&userid=" + associatedAccounts[i].replace(":", "/"));
        }
